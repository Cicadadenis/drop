...
async def upload_file(message: types.Message, state: FSMContext):
    ...
    if message.text:
        if message.text.lower() == 'отмена':
            await bot.send_message(chat_id=message.chat.id, text='Ты вернулись в главное меню.', reply_markup=kb.menu_kb())
            await state.finish()
        else:
            ...
            if message.text == password[0]:
                ...
                await bot.send_message(chat_id=message.chat.id, text='Это не верный пароль, попробуй еще раз:', reply_markup = kb.back_kb())
    else:
        await bot.send_message(chat_id=message.chat.id, text='Это не верный пароль, попробуй еще раз:', reply_markup = kb.back_kb())
...
async def create_post(message: types.Message):
    if db.get_users_exist(message.chat.id) == True:
        ...
        if all_types == []:
            await bot.send_message(message.chat.id, text='У Тебя нету загруженных файлов, чтобы загрузить файлы нажмите "Загрузить файл"', reply_markup = kb.menu_kb())
        else:
            text='Ваши файлы: \n\n'
...
async def upload_file(message: types.Message, state: FSMContext):
    ...
    if message.photo:
        ...
        await bot.send_message(chat_id=message.chat.id, text='Введи пароль, для файла. Если пароль не нужен, то напиши "-".', reply_markup=kb.back_kb())
...
async def del_file(message: types.Message, state: FSMContext):
    try:
        ...
        if number > len(all_ids):
            ...
            await bot.send_message(chat_id=message.chat.id, text='Ты успешно удалили файл!', reply_markup=kb.menu_kb())
            await state.finish()
    except ValueError:
...
async def handler_call(call: types.CallbackQuery, state: FSMContext):
    ...
    if call.data == 'delete_file':
        ...
        if all_ids == []:
            ...
            await bot.send_message(chat_id=chat_id, text='У Тебя нету загруженных файлов, чтобы загрузить файлы нажмите "Загрузить файл"', reply_markup = kb.menu_kb())
        else:
